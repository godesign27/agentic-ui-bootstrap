You are working in the repository: bootstrap-agentic-ai.

## Task: [FEATURE NAME]

**Scope**: Complex or multi-page feature.

## Constraints

- Separate UI composition from backend logic assumptions.
- State all assumptions explicitly before implementation.
- Reference `/patterns/` and `/examples/` extensively.
- Follow all governance files (AGENT_RULES.md, DESIGN_PRINCIPLES.md, ACCESSIBILITY.md).

## Assumptions (State Explicitly)

1. [Assumption about data/backend]
2. [Assumption about user flow]
3. [Assumption about state management]
4. [Other assumptions]

## Implementation Plan

### UI Composition
- Pattern(s): [list]
- Component(s): [list]
- Page(s): [list]

### Backend Logic (if applicable)
- [Brief description of logic assumptions]

## Output Format

1. Assumptions stated: [yes/no]
2. Pages created: [list]
3. Patterns used: [list]
4. Components used: [list]
5. Stakeholder summary: [2-3 sentence summary]

## Deliverable

[Specific deliverable description]

## Stakeholder Summary

[Brief, non-technical summary for stakeholders]

