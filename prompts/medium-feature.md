You are working in the repository: bootstrap-agentic-ai.

## Task: [FEATURE NAME]

**Scope**: Multi-component or single-page feature.

## Constraints

- Reference `/patterns/` for applicable UX patterns before composing.
- Use components from `/components/` only. No new component creation.
- Follow accessibility requirements from ACCESSIBILITY.md.
- Compose from existing snippets. Do not generate new HTML structures.
- Check `/examples/` for similar implementations.

## Required Checks

- [ ] Accessibility: WCAG 2.1 AA compliance verified
- [ ] Pattern alignment: Referenced applicable pattern(s) from `/patterns/`
- [ ] Component usage: Only used components from `/components/`
- [ ] No custom CSS: Only tokens.css, brand.css (if exists), overrides.css

## Output Format

1. Pattern(s) referenced: [list]
2. Component(s) used: [list]
3. File(s) created: [list]
4. Accessibility notes: [brief]

## Deliverable

[Specific deliverable description]

